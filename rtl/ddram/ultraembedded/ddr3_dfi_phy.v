...
module ddr3_dfi_phy
    ...
    #(
     ...
     (
         ...
         ,input           clk_ddr_i              // DDR logic clock
         ,input           clk_ddr90_i            // DDR logic clock 90 degree phase behind clk_ddr_i
         ,input           clk_ref_i              // Idle clock 1/2 of clk_ddr_i
         ,input           rst_i                  // Active high
         ,input           cfg_valid_i            // active when asserted
         ,input  [ 31:0]  cfg_i                  // DFI Configuration (DDR_PHY_CFG)
         // Control intreface to DFI
         ,input  [ 14:0]  dfi_address_i          // pN
         ,input  [  2:0]  dfi_bank_i             // pN
         ,input           dfi_cas_n_i            // pN
         ,input           dfi_cke_i              // pN
         ,input           dfi_cs_n_i             // pN
         ,input           dfi_odt_i              // pN
         ,input           dfi_ras_n_i            // pN
         ,input           dfi_reset_n_i          // pN
         ,input           dfi_we_n_i             // pN

         // Write data interface
         ,input  [ 31:0]  dfi_wrdata_i           // pN write data from the MC to thePHY
         ,input           dfi_wrdata_en_i        // pN This signal indicates to the PHY that valid dfi_wrdata will be transmitted
         ,input  [  3:0]  dfi_wrdata_mask_i      // pN Write data byte mask

         // Read data interface
         ,input           dfi_rddata_en_i        // pN This signal indicates to the PHY that a read operation to memory is underway

         // Outputs
         // Read data interface
         ,output [ 31:0]  dfi_rddata_o           // pN Read data from the PHY to the MC
         ,output          dfi_rddata_valid_o     // wN Read data valid indicator, active high, every cycle a byte
         ,output [  1:0]  dfi_rddata_dnv_o       // wN Used with LPDDR2 DRAM only, unused here
         // DDR3 interface
         ,output          ddr3_ck_p_o
         ,output          ddr3_ck_n_o
         ,output          ddr3_cke_o
...
`define DDR_PHY_CFG_DLY_DQ_INC_R    23:22
    ...
    always @ (posedge clk_i )
        if (rst_i)
            ...
            case (rd_sel_q)
                4'd0:
                    rd_capture_q <= {rd_data1_w, rd_data0_w};
                4'd1:
                    rd_capture_q <= {rd_data1_w, rd_data1_w};
                4'd2:
                    rd_capture_q <= {rd_data1_w, rd_data2_w};
                4'd3:
                    rd_capture_q <= {rd_data1_w, rd_data3_w};
                4'd4:
                    rd_capture_q <= {rd_data2_w, rd_data0_w};
                4'd5:
                    rd_capture_q <= {rd_data2_w, rd_data1_w};
                4'd6:
                    rd_capture_q <= {rd_data2_w, rd_data2_w};
                4'd7:
                    rd_capture_q <= {rd_data2_w, rd_data3_w};
                4'd8:
                    rd_capture_q <= {rd_data3_w, rd_data0_w};
                4'd9:
                    rd_capture_q <= {rd_data3_w, rd_data1_w};
                4'd10:
                    rd_capture_q <= {rd_data3_w, rd_data2_w};
                4'd11:
                    rd_capture_q <= {rd_data3_w, rd_data3_w};
                4'd12:
                    rd_capture_q <= {rd_data0_w, rd_data0_w};
                4'd13:
                    rd_capture_q <= {rd_data0_w, rd_data1_w};
                4'd14:
                    rd_capture_q <= {rd_data0_w, rd_data2_w};
                4'd15:
                    rd_capture_q <= {rd_data0_w, rd_data3_w};
            endcase
        end

...
- **Status: PORTED 2026-09-19**, commit `6e6ec15`. Built into `build/stage_ap040_demo1`; lints clean, **not yet run on hardware**. CNT is tied high at both CIAs in `minimig.v`, so the live change is only that a mode which used to count the wrong thing now counts nothing. Test: Crystal Kingdom Dizzy [cr FLT]. See `findings/aga-chipset/2026-09-19-demo-fix-port.md`.

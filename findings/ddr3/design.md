...
| D8 | **~~The "leftover" Z3 board (`sel_z3ram3`, 0x41000000) is disabled when DDR3 is present.~~ Superseded: that board *is* the DDR3 board.** | Boards 1 and 2 now stay on the SDRAM and board 3 carries the DDR3, decoded against the base the OS assigns, so the DDR3 adds memory instead of taking it away. The 4 MB of SDRAM that board used to offer is given up in the DDR3 build; getting it back needs a 4th board in the free `acdevice = 3'b100` slot. See [z3ram3-on-ddr3-plan.md](z3ram3-on-ddr3-plan.md). |

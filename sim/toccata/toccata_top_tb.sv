...
initial begin
    #1000000 $fatal(1, "Simulation took too long, failing");
end
...
typedef enum int {
    ...
    STOP_PLAYBACK,
    // 8-bit stereo
    SET_PLAYBACK_FORMAT_IDX_8_ST,
    SET_PLAYBACK_FORMAT_8_ST,
    START_PLAYBACK_8_ST,
    LOAD_SAMPLES_8BIT_ST_L,
    LOAD_SAMPLES_8BIT_ST_R,
    STOP_PLAYBACK_8_ST,
    // 16-bit mono
    SET_PLAYBACK_FORMAT_IDX_16_MONO,
    SET_PLAYBACK_FORMAT_16_MONO,
    START_PLAYBACK_16BIT_MONO,
    LOAD_SAMPLES_16BIT_MONO_LSB,
    LOAD_SAMPLES_16BIT_MONO_MSB,
    // Done
    FINISH
} state_t;
state_t state;
...
int        pos_counter, pos_counter_next;
    ...
    if (rst) begin
        ...
        case (state)
            ...
            STOP_PLAYBACK : begin
                ...
                if (lwr == 1'b1) begin
                    lwr <= 1'b0;
                    state <= SET_PLAYBACK_FORMAT_IDX_8_ST;
                end
            end
            // 8-bit stereo audio
            SET_PLAYBACK_FORMAT_IDX_8_ST : begin
                addr <= 16'h6700;
                hwr <= 1'b1;
                lwr <= 1'b0;
                rd <= 1'b0;
                data_in <= {8'h08, 8'h00};
                if (hwr == 1'b1) begin
                    state <= SET_PLAYBACK_FORMAT_8_ST;
                    hwr <= 1'b0;
                end
            end
            SET_PLAYBACK_FORMAT_8_ST : begin
                addr <= 16'h6800;
                hwr <= 1'b1;
                lwr <= 1'b0;
                data_in <= {8'h18, 8'h00}; // 24 MHz clock, 8kHz, stereo, 8-bit
                if (hwr == 1'b1) begin
                    state <= START_PLAYBACK_8_ST;
                    hwr <= 1'b0;
                end
            end
            START_PLAYBACK_8_ST : begin
                // Set the bit to start playback to one
                addr <= 16'h0000; // Status register
                hwr <= 1'b0;
                lwr <= 1'b1;
            ...
            LOAD_SAMPLES_8BIT_ST_R : begin
                ...
                if (pos_counter > 512) begin
                    // Buffer is full we are done
                    state <= SET_PLAYBACK_FORMAT_IDX_16_MONO;
                    lwr <= 1'b0;
                end
            end
            // 16-bit mono test
            SET_PLAYBACK_FORMAT_IDX_16_MONO: begin
                addr <= 16'h6700;
                hwr <= 1'b1;
                lwr <= 1'b0;
                rd <= 1'b0;
                data_in <= {8'h08, 8'h00};
                if (hwr == 1'b1) begin
                    state <= SET_PLAYBACK_FORMAT_16_MONO;
                    hwr <= 1'b0;
                end
            end
            SET_PLAYBACK_FORMAT_16_MONO: begin
                addr <= 16'h6800;
                hwr <= 1'b1;
                lwr <= 1'b0;
                data_in <= {8'h48, 8'h00}; // 24 MHz clock, 8kHz, stereo, 16-bit
                if (hwr == 1'b1) begin
            ...
            LOAD_SAMPLES_16BIT_MONO_LSB: begin
                // Load a sequence of samples
                addr <= 16'h2000;
                hwr <= 1'b0;
                lwr <= 1'b1;
                rd <= 1'b0;
                data_in <= {8'h00, audio_in_16_l[7:0]};
                if (lwr == 1'b1) begin
                    lwr <= 1'b0;
                    state <= LOAD_SAMPLES_16BIT_MONO_MSB;
                end
            end
            LOAD_SAMPLES_16BIT_MONO_MSB: begin
                // Load a sequence of samples
                addr <= 16'h2000;
                hwr <= 1'b0;
                lwr <= 1'b1;
                rd <= 1'b0;
                data_in <= {8'h00, audio_in_16_l[15:8]};
                if (lwr == 1'b1) begin
                    pos_counter <= pos_counter_next;
                    lwr <= 1'b0;
                    state <= LOAD_SAMPLES_16BIT_MONO_LSB;

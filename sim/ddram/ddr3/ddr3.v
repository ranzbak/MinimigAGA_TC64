    ...
    task chk_err;
        ...
        begin
            ...
            casex ({truebl4, relationship, fromcmd, cmd})
                ...
                {1'bx, DIFF_BANK , LOAD_MODE, SELF_REF } : begin
                    if (($time - tm_load_mode < TMOD-TJIT_PER) || (ck_cntr - ck_load_mode < TMOD_TCK))
                        $display ("%m: at time %t ERROR:  tMOD violation during %s", $time, cmd_string[cmd]);
                end
                ...
                {1'bx, DIFF_BANK , REFRESH  , SELF_REF } : begin
                    if ($time - tm_refresh < TRFC_MIN)
                        $display ("%m: at time %t ERROR:  tRFC violation during %s (%t < %t)", $time, cmd_string[cmd],$time - tm_refresh,TRFC_MIN);
                end
                {1'bx, DIFF_BANK , REFRESH  , PWR_DOWN } : begin
                    if (ck_cntr - ck_refresh < TREFPDEN)
                        $display ("%m: at time %t ERROR:  tREFPDEN violation during %s", $time, cmd_string[cmd]);
                end

                // precharge
                {1'bx, SAME_BANK , PRECHARGE, ACTIVATE } : begin
                ...
                // activate
                {1'bx, SAME_BANK , ACTIVATE , PRECHARGE} : begin
                ...
                {1'bx, SAME_BANK , ACTIVATE , ACTIVATE } : begin
                ...
                {1'b0, DIFF_BANK , ACTIVATE , ACTIVATE } : begin
                ...
                {1'bx, DIFF_BANK , WRITE    , PWR_DOWN } : begin
                ...
                // read
                {1'bx, SAME_BANK , READ     , PRECHARGE} : begin
                ...
                {1'b1, DIFF_BANK , READ     , WRITE    } :
                    ; // tRTW is checked outside this task
                {1'b0, DIFF_BANK , READ     , READ     } : begin
                    if (ck_cntr - ck_read < TCCD)
                        $display ("%m: at time %t ERROR:  tCCD violation during %s to bank %d", $time, cmd_string[cmd], bank);
                end
                {1'b1, DIFF_BANK , READ     , READ     } : begin
                    if (ck_cntr - ck_group_read[bank[1]] < TCCD)
                        $display ("%m: at time %t ERROR:  tCCD violation during %s to bank %d", $time, cmd_string[cmd], bank);
                end
                {1'b1, DIFF_GROUP, READ     , WRITE    } :
                    ; // tRTW is checked outside this task
                {1'b1, DIFF_GROUP, READ     , READ     } : begin
                    if (ck_cntr - ck_read < TCCD_DG)
                        $display ("%m: at time %t ERROR:  tCCD_DG violation during %s to bank %d", $time, cmd_string[cmd], bank);
                end
                {1'bx, DIFF_BANK , READ     , PWR_DOWN } : begin
                ...
                {1'bx, DIFF_BANK , PWR_DOWN , ZQ       } : begin
                ...
                {1'bx, DIFF_BANK , PWR_DOWN , READ     } : begin
                    if (($time - tm_power_down < TXP) || (ck_cntr - ck_power_down < TXP_TCK))
                        $display ("%m: at time %t ERROR:   tXP violation during %s", $time, cmd_string[cmd]);
                ...
                {1'bx, DIFF_BANK , PWR_DOWN , SELF_REF } : begin
                    if (($time - tm_power_down < TXP) || (ck_cntr - ck_power_down < TXP_TCK))
                        $display ("%m: at time %t ERROR:   tXP violation during %s", $time, cmd_string[cmd]);
                ...
                {1'bx, DIFF_BANK , SELF_REF , ZQ       } : begin
                ...
                {1'bx, DIFF_BANK , SELF_REF , SELF_REF } : begin
                    if (($time - tm_self_refresh < TXS) || (ck_cntr - ck_self_refresh < TXS_TCK))
                        $display ("%m: at time %t ERROR:   tXS violation during %s", $time, cmd_string[cmd]);
                    if (($time - tm_cke_cmd < TCKE) || (ck_cntr - ck_cke_cmd < TCKE_TCK))
                        $display ("%m: at time %t ERROR:  tCKE violation on CKE", $time);
            ...
            if (cke) begin
                ...
                case (cmd)
                        ...
                        if (|active_bank) begin
                            ...
                            case (bank)
                                ...
                                1 : begin
                                    ...
                                    else if ((odt_rtt_nom < 4) || ((!addr[7] || (addr[7] && addr[12])) && (odt_rtt_nom < 6))) begin
                                        if (DEBUG)
                                    ...
                                    else if ((al >= AL_MIN) && (al <= AL_MAX)) begin
                                        if (DEBUG)
                                    ...
                                    else if (write_levelization) begin
                                        if (DEBUG)
                                ...
                                2 : begin
                                    ...
                                    else if ((odt_rtt_wr > 0) && (odt_rtt_wr < 3)) begin
                                        if (DEBUG)
                    ...
                    ACTIVATE : begin
                        ...
                        else if (active_bank[bank]) begin
                    ...
                    WRITE : begin
                        ...
                        else if (auto_precharge_bank[bank]) begin
                            $display ("%m: at time %t ERROR: %s Failure.  Auto Precharge is scheduled to bank %d.", $time, cmd_string[cmd], bank);
                            if (STOP_ON_ERROR)
                                $stop(0);
                        end
                        else if ((ck_cntr - ck_write < burst_length/2) && (truebl4 == 0)) begin
...
`endif
                    ...
                    READ : begin
                        ...
                        else if (!active_bank[bank] && !mpr_en) begin
                            if (check_strict_timing)
                                $display ("%m: at time %t ERROR: %s Failure.  Bank %d must be Activated.", $time, cmd_string[cmd], bank);
                            if (STOP_ON_ERROR)
                                $stop(0);
                        ...
                        else if ((ck_cntr - ck_read < burst_length/2) && (truebl4 == 0)) begin
                            $display ("%m: at time %t ERROR: %s Failure.  Illegal burst interruption.", $time, cmd_string[cmd]);
                            if (STOP_ON_ERROR)
                                $stop(0);
                        end
...
`endif
    ...
    task data_task;
        ...
        begin
            ...
            else if (rdqs_cntr > RDQS_PST) begin // read data
                dqs_out = rdqs_cntr - RDQS_PST;
            end
            else if (rdqs_cntr > 0) begin // read postamble
                dqs_out = 1'b0;
            end
            else begin
                dqs_out = 1'b1;
...
`ifdef MPR_DQ0 // DQ0 output MPR data, other DQ low
                    ...
                    else if (odts_readout && (mpr_select == 2'b11)) begin // Temp Sensor (ODTS)
        ...
        if (!rst_n_in) begin
            ...
            if (diff_ck) begin
                ...
                if (dll_locked) begin
                    ...
                    else if (init_mode_reg[0] && (mr_chk == 1)) begin
